...
class HAEFNet(nn.Module):
    ...
    def get_param_groups(self):
        ...
        for name, param in list(self.haef_encoder.named_parameters()):
            if "norm" in name:
    ...
    def forward(self, x):
        ...
        if not self.use_evidential_fusion:
            return self.haef_encoder(x)

        modality_features = self.haef_encoder.encoder(x)
        ...
        for s in range(num_stages):
            ...
            if self.keep_pca_before_gem:
                ...
                for m in range(num_modalities):
                    ...
                    y_m, y_n = self.haef_encoder.pca_stages[s](updated_seqs[m], updated_seqs[nxt])
        ...
        for t in range(self.num_parallel):
            ...
            modal_logit_t = torch.log(pl_t + eps)
            pl_list.append(pl_t)
            ...
            if modal_logit_up_t.shape[2:] != original_shape:
                pl_up = F.interpolate(pl_t, size=original_shape, mode="bilinear", align_corners=False)
                modal_logit_up_t = torch.log(pl_up + eps)
        ...
        logits = torch.log(pl_fused + eps)
        if pl_fused.shape[2:] != original_shape:
            pl_up = F.interpolate(pl_fused, size=original_shape, mode="bilinear", align_corners=False)
            logits = torch.log(pl_up + eps)
    ...
    def _decode_from_features(self, modality_features, original_shape):
        ...
        head_logits = self.haef_encoder.decoder(fused_features)
