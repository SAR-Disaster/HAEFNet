...
class CompositeLoss(nn.Module):
    ...
    def forward(self, outputs, labels, *, modal_logits=None, theta=None, beta=None, product_logprob=None):
        ...
        fused_logits = _resize(outputs, target_hw)
        fused_logprob = _softmax_logits(fused_logits)
        ...
        if self.modal_weight > 0 and modal_logits:
            ...
            for logit in modal_logits:
                ...
                modal_total += self._task_loss(_softmax_logits(logit), labels)
...
def build_loss(config: Dict[str, Any]) -> nn.Module:
    ...
    task = loss_cfg.get("task", None)
    modal = loss_cfg.get("modal", None)
    uncertainty = loss_cfg.get("uncertainty", None)
    beta_reg = loss_cfg.get("beta_reg", None)
    fuse_consistency = loss_cfg.get("fuse_consistency", None)
